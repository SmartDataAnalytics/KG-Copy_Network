...
class Mem2Seq(nn.Module):
    ...
    def save_model(self, dec_type):
        ...
        directory = 'save/mem2seq-'+name_data+str(self.task)+'HDD'+str(self.hidden_size)+'BSZ'+str(args['batch'])+'DR'+str(self.dropout)+'L'+str(self.n_layers)+'lr'+str(self.lr)+'F1'+str(self.f1)+str(dec_type)
    ...
    def evaluate(self,dev,avg_best,BLEU=False):
        ...
        if args['dataset'] == 'kvr':
            with open('data/soccer_entities.txt','r',encoding="utf-8") as f:
                for line in f:
                    global_entity_list.add(line.strip())
        ...
        epsilon = 0.00001
        if args['dataset']=='kvr':
            logging.info("F1 SCORE:\t{}".format(microF1_TRUE/float(microF1_PRED+epsilon)))
            #logging.info("\tCAL F1:\t{}".format(microF1_TRUE_cal/float(microF1_PRED_cal+epsilon)))
            #logging.info("\tWET F1:\t{}".format(microF1_TRUE_wet/float(microF1_PRED_wet+epsilon)))
            logging.info("\tNAV F1:\t{}".format(microF1_TRUE_nav/float(microF1_PRED_nav+epsilon)))
            self.f1= microF1_TRUE_nav/float(microF1_PRED_nav+epsilon)
        elif args['dataset']=='babi' and int(args["task"])==6:
